...
async function getSummary(req, res) {
    try {
        const [
            ...
            Log.countDocuments(),
            Log.countDocuments({ action: 'Blocked' }),
            Log.countDocuments({ severity: 'Critical', action: 'Blocked' }),
        ...
        res.json({
            ...
            data: {
                ...
                last24hBlocked,
                uptime: '99.98%',
                rpsNow: Math.floor(Math.random() * 80 + 40),
...
async function getTraffic(req, res) {
    try {
        // Aggregate logs by 2-hour buckets over last 24h
        const since = new Date(Date.now() - 24 * 60 * 60 * 1000)
        ...
        // Build full 12-slot array (00:00 to 22:00)
        const slots = Array.from({ length: 12 }, (_, i) => {
            const h = i * 2
            const label = `${String(h).padStart(2, '0')}:00`
            const found = agg.find(a => a._id.hour === h)
            return {
                label,
                requests: found ? found.requests : Math.floor(Math.random() * 200 + 50),
                blocked: found ? found.blocked : Math.floor(Math.random() * 30 + 5),
            }
        })
...
async function getThreatBreakdown(req, res) {
    try {
        ...
        const data = breakdown.map(b => ({
            ...
            color: COLOR_MAP[b._id] || '#8899b0',
        }))

        res.json({ success: true, data })
    } catch (err) {
        res.status(500).json({ success: false, error: 'Failed to fetch threat breakdown.' })
    }
}
...
async function getGeoBreakdown(req, res) {
    try {
        const geo = await Log.aggregate([
            { $match: { action: 'Blocked' } },
            { $group: { _id: { country: '$country', flag: '$countryFlag', code: '$countryCode' }, count: { $sum: 1 } } },
            { $sort: { count: -1 } },
            { $limit: 6 },
        ])

        const COLOR_MAP = {
            RU: '#ef4444', CN: '#ef4444', BR: '#f59e0b',
            IN: '#f59e0b', US: '#60a5fa', NL: '#8b5cf6',
        }
        ...
        const data = geo.map(g => ({
            ...
            color: COLOR_MAP[g._id.code] || '#3d5570',
        }))

        res.json({ success: true, data })
    } catch (err) {
        res.status(500).json({ success: false, error: 'Failed to fetch geo data.' })
    }
}
