...
// ── Flag: pass --fresh to wipe everything (e.g. node seed.js --fresh) ─────────
...
const log = {
    ...
    section: (msg) => console.log(`\n── ${msg} ${'─'.repeat(Math.max(0, 40 - msg.length))}`),
}

async function seed() {
    console.log('\n🛡️  ShieldWAF Database Seeder')
    console.log('================================')
    if (FRESH) console.log('  Mode: FRESH  (all existing data will be wiped)')
    else        console.log('  Mode: SAFE   (existing users preserved — run with --fresh to wipe all)')
    ...
    try {
        ...
        // ── Wipe (only in --fresh mode) ───────────────────────────────────────
        if (FRESH) {
            log.section('Clearing Data')
            await Promise.all([User.deleteMany({}), Rule.deleteMany({}), Log.deleteMany({})])
            log.success('All users, rules, and logs deleted')
        } else {
            log.section('Clearing Rules & Logs')
            await Promise.all([Rule.deleteMany({}), Log.deleteMany({})])
            log.success('Rules and logs cleared (users untouched)')
        }
        ...
        if (existingAdmin) {
            log.skip(`Admin already exists: ${ADMIN_EMAIL}  (skipping creation)`)
        } else {
            await User.create({
                name: 'Admin',
                email: ADMIN_EMAIL,
            ...
            log.success(`Admin created: ${ADMIN_EMAIL}`)
        }

        // ── WAF Rules ─────────────────────────────────────────────────────────
        log.section('WAF Rules')
        await Rule.insertMany(DEFAULT_RULES)
        log.success(`${DEFAULT_RULES.length} rules inserted`)

        // ── Sample Logs ───────────────────────────────────────────────────────
        log.section('Sample Logs')
        const logs = Array.from({ length: 80 }, (_, i) => {
            const s  = ATTACK_SAMPLES[i % ATTACK_SAMPLES.length]
            const ip = SAMPLE_IPS[rb(0, SAMPLE_IPS.length - 1)]
            return {
        ...
        await Log.insertMany(logs)
        log.success(`${logs.length} sample logs generated`)
        ...
        console.log(`  Admin password    : ${existingAdmin ? '(unchanged)' : ADMIN_PASSWORD}`)
        console.log('════════════════════════════════════════\n')
        ...
        if (err.code === 11000) {
            console.error('   Duplicate key — try running with --fresh flag:')
            console.error('   node src/utils/seed.js --fresh\n')
        }
        process.exit(1)
    }
}
